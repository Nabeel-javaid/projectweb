        ...
        {sortedData.map((item, index) => (
          <div className="w-[70%] h-60 border-orange-500 shadow-lg rounded m-7 flex flex-row p-4" key={item.id}>
            ...
            <div id="logo" className="w-[18%] flex items-center justify-center border-r px-4 solid">
              <Image src={logos[index % logos.length]} width={200} />
            </div>
...
export async function getServerSideProps(context) {
  ...
  // Modify the query to use .ilike() for case-insensitive matching
  const { data, error } = await supabase
    .from("bookings")
    .select("*")
    .ilike("departure_airport", `%${departureCity}%`)
    .ilike("arrival_airport", `%${arrivalCity}%`);
